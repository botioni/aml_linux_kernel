...
static const u8 blkmode_reg[] = {5, 7, 0, 1, 2, 4, 4, 4, 4};
static const u8 colormat_reg[] = {0, 0, 0, 0, 0, 0, 1, 2, 3};
static const u8 modebits[] = {32,24,2,4,8,16,16,16,16};
                  ...
                  int  index)
{
	...
	tv_scan_mode='i';
	if ( request_irq(AM_ISA_GEN_IRQ(IRQNUM_VSYNC), &vsync_isr,
    ...
    data32 |= colormat_reg[type]     << 13;
    data32 |= OSD_TC_ALPHA_ENABLE_DEF   << 15;
    data32 |= 1                      << 16; /* rgb enable */
    data32 |= blkmode_reg[type]      << 17; /* osd_blk_mode */
...
{
		u8  r,g,b,a=(colorkey&0xff000000)>>24;
...
{
	u32  reg=VIU_OSD1_BLK0_CFG_W3 + REG_OFFSET*index;
                ...
                )
{
    ...
    /* configure canvas */
    printk("osd%d:xres_virtual: %d, yres_virtual: %d, mem: 0x%x\n",index, xres_virtual, yres_virtual, fbmem);
    printk("osd%d:type = %d, bpp = %d, canvas width = %d,canvas_index=%d\n",index, type, modebits[type], w,osd_ctl->index);

    canvas_config(osd_ctl->index, osd_ctl->addr,
					w, osd_ctl->yres_virtual,
					CANVAS_ADDR_NOWRAP, CANVAS_BLKMODE_LINEAR);

    /* configure level-3 canvas */
    if(index==0) //osd1
    {
    	WRITE_MPEG_REG(DDR_CANVAS_CTL, 0x15);               /* enable IO space 0xa000_0000 */
    	WRITE_MPEG_REG(DDR_CANVAS_GRANT, 1<<25);            /* enable CPU IO space access */
    	WRITE_MPEG_REG(DDR_CANVAS_DATA_FLIP, 1<<25);        /* set CPU IO access endian */
    	WRITE_MPEG_REG_BITS(DDR_CANVAS_IDX0, osd_ctl->index,
                       0, 8);                           /* hook level3 canvas with canvas */
    }
    else if(index==1) //osd2
    {
      	WRITE_MPEG_REG(DDR_CANVAS_CTL, 0x15);               /* enable IO space 0xa000_0000 */
    	WRITE_MPEG_REG(DDR_CANVAS_GRANT, 1<<25);            /* enable CPU IO space access */
    	WRITE_MPEG_REG(DDR_CANVAS_DATA_FLIP, 1<<25);        /* set CPU IO access endian */
    ...
    _init_osd_simple( xoffset, xoffset + (disp_end_x-disp_start_x)  ,
                		      yoffset, yoffset + (disp_end_y-disp_start_y)   ,disp_start_x,disp_start_y,
                		      disp_end_x,disp_end_y,osd_ctl->index, type,index);
	SET_MPEG_REG_MASK(VIU_OSD1_CTRL_STAT, 1 << 21);
	WRITE_MPEG_REG_BITS(VIU_OSD1_FIFO_CTRL_STAT, 0x2, 10, 2); //set burst size 48
	WRITE_MPEG_REG(VPP_MISC, (1<<13)|(1<<12)|(1<<7)|(0<<4));
}
...
void osd_enable(int enable ,int index )
{
    ...
    int  enable_bit=index? (1<<13) :(1<<12) ;	//osd1 osd2 select bit.
...
void osd_pan_display(unsigned int xoffset, unsigned int yoffset,int index )
{
    ...
    cfgw0 = READ_MPEG_REG(VIU_OSD1_BLK0_CFG_W0+REG_OFFSET*index);
    pix_y_end = cfgw0 & 0x1fff;
    pix_x_end = (cfgw0 >> 13) & 0x1fff;
    pix_y_start = cfgw0 >> 26;

    cfgw1 = READ_MPEG_REG(VIU_OSD1_BLK0_CFG_W1+REG_OFFSET*index);
    pix_y_start |= (cfgw1 & 0x7f) << 6;
    pix_x_start = (cfgw1 >> 7) & 0x1fff;
    
    pix_x_end += xoffset - pix_x_start;
    pix_x_start = xoffset;
    pix_y_end += yoffset - pix_y_start;
    pix_y_start = yoffset;
    
    cfgw0  = pix_y_end            << 0;
    cfgw0 |= pix_x_end            << 13;
    cfgw0 |= (pix_y_start & 0x3f) << 26;
    WRITE_MPEG_REG(VIU_OSD1_BLK0_CFG_W0+REG_OFFSET*index, cfgw0);

    cfgw1 &= ~0xfffff;
    cfgw1 |= (pix_y_start >> 6)     << 0;
    cfgw1 |= pix_x_start            << 7;
    WRITE_MPEG_REG(VIU_OSD1_BLK0_CFG_W1+REG_OFFSET*index, cfgw1);
}
/********************************************************
*  request vsync irq  *****************************************
*********************************************************/
