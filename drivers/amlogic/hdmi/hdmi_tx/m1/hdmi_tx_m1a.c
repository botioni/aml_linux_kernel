...
const static char hdmi_tx_id[] = "amhdmitx-id";
...
static irqreturn_t intr_handler(int irq, void *dev_instance)
{
    ...
    //data32 = Rd(A9_0_IRQ_IN1_INTR_STAT);
    //if (data32 & (1 << 25)) {
        ...
        if (data32 & (1 << 0)) {  //HPD rising 
            ...
            // Error if HPD deasserts
                ...
                //stimulus_print("[TEST.C] Error: HDMI HPD deasserts!\n");
            ...
            //stimulus_print("[TEST.C] Error: Unkown HDMI Interrupt source Process_Irq\n");
...
int HDMITX_HW_SetMode(Hdmi_tx_video_para_t *param)
{
    int ret=0;
    if(param->VIC==HDMI_480p60){
...
void HDMITX_HW_SetAVI(unsigned char* AVI_DB, unsigned char* AVI_HB)
{
    // AVI frame
    int i ;
...
void HDMITX_HW_SetAudioInfoFrame(unsigned char bEnable, unsigned char* AUD_DB)
{
...
int HDMITX_HW_GetEDIDData(int block, unsigned char* buf)
{
    int i;
    if(block>3){
        return 0;    
    ...
    for(i=0;i<128;i++){
        buf[i]=hdmi_rd_reg(0x600+block*128+i);
    }
    return 0;
}
    
void HDMITX_HW_SetupIRQ()
{
   int r;
