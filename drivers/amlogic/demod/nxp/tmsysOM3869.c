...
)
{
    ...
    tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "tmbslNT220xGetSWVersion failed.", 0));
...
)
{
    ...
    {
        tmDBGPRINTEx(DEBUGLVL_TERSE,("Configuring Tuner!"), 0);
    ...
    if(err == TM_OK)
    {
        ...
        if (err != TM_OK)
        {
            ...
            tmDBGPRINTEx(DEBUGLVL_ERROR, "tmbslNT220xSetStandardMode(0x%08X) failed.", pObj->tUnitNT220x);
        }
    }
    ...
    if(err == TM_OK)
    {
        ...
        if (err != TM_OK)
        {
			...
			tmDBGPRINTEx(DEBUGLVL_ERROR, "tmbslNT220xSetRf(0x%08X) failed.", pObj->tUnitNT220x);
        }
    }
    ...
    if(err == TM_OK)
    {
        ...
        tmDBGPRINTEx(DEBUGLVL_TERSE, "NT220x(0x%08X) PLL Lock:.", pObj->tUnitNT220x);
    }
    if(err == TM_OK)
    {
...
)
{
    ...
    if( !pTuneRequest )
    {
        tmDBGPRINTEx(DEBUGLVL_ERROR, "Error: Invalid Pointer!", 0);
    ...
    if(err == TM_OK)
    {
        ...
        case TRT_TUNER_ONLY:
            ...
            {
                tmDBGPRINTEx(DEBUGLVL_ERROR, "Error: Bad parameter", 0);
                err = OM3869_ERR_BAD_PARAMETER;
            }
            else
            {
                if (err == TM_OK)
                {
                    ...
                    tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "Error: OM3869SetFrequency failed", 0));
                }
            }
            break;

        default:
            tmDBGPRINTEx(DEBUGLVL_ERROR, "Error: Unsupported tune request", 0);
            err = OM3869_ERR_NOT_SUPPORTED;
            break;
        }
...
)
{
    ...
    {
        tmDBGPRINTEx(DEBUGLVL_TERSE,("Error: Invalid Pointer!"), 0);
        err = OM3869_ERR_BAD_PARAMETER;
    }
