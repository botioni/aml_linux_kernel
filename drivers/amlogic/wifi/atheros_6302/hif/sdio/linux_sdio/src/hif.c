...
extern int sdio_io_rw_extended(struct memory_card *card, int write, unsigned fn,
...
return 0;
    ...
    do {
        ...
        /* Issue CMD5, arg = 0 */
        err = IssueSDCommand(device, SD_IO_SEND_OP_COND, 0, (MMC_RSP_R4 | MMC_CMD_BCR), NULL);
        if (err) {
...
static A_STATUS hifDisableFunc(HIF_DEVICE *device, struct sdio_func *func)
{
    ...
    if (reset_sdio_on_unload) {
     ...
     //   ret = Func0_CMD52WriteByte(device->func->card, SDIO_CCCR_ABORT, (1 << 3)); 
		ret = Func0_CMD52WriteByte(device->func, SDIO_CCCR_ABORT, (1 << 3)); 
        if (ret) {
            status = A_ERROR;
...
static int hifEnableFunc(HIF_DEVICE *device, struct sdio_func *func)
{
    ...
    if (device->is_disabled) {
	  ...
	  //ret = Func0_CMD52WriteByte(func->card, CCCR_SDIO_IRQ_MODE_REG, SDIO_IRQ_MODE_ASYNC_4BIT_IRQ);
            ret = Func0_CMD52WriteByte(func, CCCR_SDIO_IRQ_MODE_REG, SDIO_IRQ_MODE_ASYNC_4BIT_IRQ);
            if (ret) {
